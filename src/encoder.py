...
def bytes_to_unicode():
    ...
    vocab if you want to avoid UNKs. When you're at something like 
    a 10B token dataset you end up needing around 5K for decent 
    coverage. This is a signficant percentage of your normal, 
    say, 32K bpe vocab. To avoid that, we want lookup tables between 
    ...
    def bpe(self, token):
        ...
        while True:
            bigram = min(
                pairs, 
                key = \
                    lambda pair: self.bpe_ranks.get(pair, float('inf'))
            )
...
def get_encoder(model_name, models_dir):
    with open(os.path.join(models_dir, model_name, 'encoder.json'), 
              'r') as f:
        encoder = json.load(f)
